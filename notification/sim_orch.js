...
sim_event.on('arrival', function(current){
  ...
  for (var j = 0; j < config.service; j += 1) {
    stats.push(nextPareto(config.min/10, config.shape));
    success += 1;
  }
  for(var i = 0; i < config.partition; i += 1) {
    if (network_state[i] === 1) {
      for (j = 0; j < config.service; j += 1) {
        stats.push(nextPareto(config.min, config.shape));
        success += 1;
      }
    } else {
  ...
  // process.nextTick(sim_event.emit('next'));
  sim_event.emit('next');
});
